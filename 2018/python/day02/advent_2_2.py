...
axcye
wvxyz
The IDs abcde and axcye are close, but they differ by two characters (the second and fourth). However, the IDs fghij and fguij differ by exactly
  ...
  for box in boxes:
    ...
    for i in xrange(len(box)):
      for c in 'abcdefghijklmnopqrstuvwxyz':
        one_off = box[0:i] + c + box[(i+1):]
        if one_off in d:
          #print('found', sorted(d.keys()))
          print(one_off, box[0:i] + box[(i+1):], i, c, d[one_off])
          return d[one_off].replace('*', '')
        add_us.append((one_off, box[0:i] + '*' + box[(i+1):]))
    for add_me, with_star in add_us:
      d[add_me] = with_star
  return None
