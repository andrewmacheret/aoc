...
def parse(logs):
  ...
  parsed_log = None
  for log in logs:
    parts = re.match(r'^\[([0-9]+)-([0-9]+)-([0-9]+) ([0-9]+):([0-9]+)\] (.+)$', log)
    action = parts.group(6)
    if action[0] == 'G': #guard
      guard_id = re.match(r'^Guard #([0-9]+) begins shift$', action).group(1)
    parsed_action = 'start' if action[0] == 'G' else 'sleep' if action[0] == 'f' else 'wake'
    ...
    if parsed_log:
      if parsed_action == 'start' and parsed_log['action'] == 'sleep': raise Exception('sleep without wake')
      if parsed_action == 'wake' and parsed_log['action'] == 'wake': raise Exception('double wake')
      if parsed_action == 'sleep' and parsed_log['action'] == 'sleep': raise Exception('double sleep')
      if parsed_action == 'wake' and parsed_log['action'] == 'sleep':
        if year != parsed_log['year']: raise Exception('year differs')
        if month != parsed_log['month']: raise Exception('month differs')
        if day != parsed_log['day']: raise Exception('day differs')
        if hour != parsed_log['hour']: raise Exception('hour differs')
        if minute <= parsed_log['minute']: raise Exception('minute is wacko')
    ...
    out.append(parsed_log)
  return out


def solve(logs):
  parsed_logs = parse(sorted(logs))
  sleep_times = defaultdict(int)
  for log in parsed_logs:
    if log['action'] == 'sleep':
      sleep_start = log['minute']
    elif log['action'] == 'wake':
      sleep_times[log['guard_id']] += log['minute'] - sleep_start
  
  best_sleep_time = -1
  best_guard_id = -1
  for guard_id, sleep_time in sleep_times.iteritems():
    if best_sleep_time < sleep_time:
      best_sleep_time = sleep_time
      best_guard_id = guard_id

  times_asleep = [0 for minute in xrange(60)]
  for log in parsed_logs:
    if log['guard_id'] == best_guard_id:
      if log['action'] == 'sleep':
        sleep_start = log['minute']
      elif log['action'] == 'wake':
        sleep_end = log['minute']
        for minute in xrange(sleep_start, sleep_end):
  ...
  best_minute = -1
  best_times_asleep = -1
  for minute, times_asleep in enumerate(times_asleep):
    if best_times_asleep < times_asleep:
      best_times_asleep = times_asleep
      best_minute = minute
      print('found better')
    elif best_times_asleep == times_asleep:
      print('found equal')

  print(best_guard_id, best_sleep_time, best_minute, best_times_asleep)

  return int(best_guard_id) * best_minute
