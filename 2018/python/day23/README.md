...
--- Day 23: Experimental Emergency Teleportation ---
   ...
   very specific formation. The device lists the X,Y,Z position (pos) for
   each nanobot as well as its signal radius (r) on its tiny screen (your
   puzzle input).
   ...
   measured by [14]Manhattan distance). Coordinates a distance away of
   less than or equal to a nanobot's signal radius are said to be in range
   of that nanobot.
   ...
   and then, for that nanobot, the total number of nanobots that are in
   range of it, including itself.
