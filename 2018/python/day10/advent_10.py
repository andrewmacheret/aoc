...
def parse(instructions):
  ...
  for instruction in instructions:
    match = re.match(r'^position=< *(-?[0-9]+), *(-?[0-9]+)> velocity=< *(-?[0-9]+), *(-?[0-9]+)>$', instruction)
    particles.append(Particle(int(match.group(1)), int(match.group(2)), int(match.group(3)), int(match.group(4))))
  return particles
...
def retreat(particles):
  #print 'Retreat!'
  for particle in particles:
    particle.retreat()

def get_width(particles):
  return max(p.x for p in particles) - min(p.x for p in particles)


def draw(particles):
  start_x, start_y = min(p.x for p in particles), min(p.y for p in particles)
  end_x, end_y = max(p.x for p in particles)+1, max(p.y for p in particles)+1
  grid = [['.' for _ in xrange(start_x, end_x)] for _ in xrange(start_y, end_y)]
  for particle in particles:
    grid[particle.y-start_y][particle.x-start_x] = '*'
  print('\n'.join(''.join(row) for row in grid))


def solve(instructions):
  particles = parse(instructions)
  #draw(particles)
  last_width = get_width(particles)

  while True:
    advance(particles)
    new_width = get_width(particles)
