...
def print_round(data, units, round, is_end=False):
    print('\nROUND ' + str(round) + (' - Done' if is_end else '') + '\n')
    alive_units_by_pos = {e.pos(): e for e in units if e.is_alive}
    for y in range(len(data)):
        out = []
        units_in_row = []
        for x in range(len(data[0])):
            if (x, y) in alive_units_by_pos:
                unit = alive_units_by_pos[(x, y)]
                out.append(unit.unit_type)
                units_in_row.append(unit.type_with_health())
            else:
                out.append(data[y][x])
        print(''.join(out) + '   ' + ', '.join(units_in_row))


def solve(d, no_elves_die):
    input_data = open("inputs/day15").read()
        ...
        for idx, c in enumerate(order):
            ...
            if enemy_positions_in_range:
                ...
                if len(alive) == 1:
                    ...
                    if idx == len(order) - 1:
                        round += 1
                    #print_round(data, units, round, True)
